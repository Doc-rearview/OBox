...
class WatchListProvider extends ChangeNotifier {
  ...
  Future<List<Entry>> list() async {
    ...
    final watchlist = await ApiClient.database.listDocuments(
      collectionId: _collectionId, databaseId: 'movies',
    );
    ...
    final movieIds = watchlist.documents
        ...
        .toList();
    final entries =
        (await ApiClient.database.listDocuments(collectionId: 'movies', databaseId: 'movies'))
            .documents
  ...
  Future<void> remove(Entry entry) async {
    ...
    await ApiClient.database
        .deleteDocument(collectionId: _collectionId, documentId: id, databaseId: 'movies',);

    list();
  }

  Future<Uint8List> imageFor(Entry entry) async {
    return await ApiClient.storage.getFileView(fileId: entry.thumbnailimageid, bucketId: 'moviess',);
  }
